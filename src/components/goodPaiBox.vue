    ...
    <template v-for="item in recommendTableData">
      <li class="good-pai-box" @click="showAuctionGoodDetails(item)">
        <div class="good-box-left">
          <img :src="'http://47.104.10.147/image/' + item.merchantId + '/' + item.picName[0]" alt="">
        </div>
        <div class="good-box-right">
          <div class="good-box-info">
            <div> {{item.productName}}</div>
            <timer :time="getTime(item.endtime)"></timer>
            <div class="price-now">当前出价：<i>¥{{item.salePrice}}</i>起 </div>
    ...
    methods: {
      showAuctionGoodDetails(item) {
        ...
        this.$router.push({
          ...
          query: {productId: item.productId,id: item.id, type: item.type}
        })
      },
      getTime (num) {
        return getTime(num)
